...
function Game(canvas, game, gridSizeW, gridSizeH) {
   ...
   if (missingDeps.length !== 0) {
      alert(
         "This browser doesn't include some of the technologies needed to play the game"
      );
      this.started = false;
      return;
   }
   ...
   this.scrollPosition.y -=
      this.grid.height * this.zoomHelper.level + this.scrollPosition.y;
   this.scrollPosition.x -=
      this.grid.width * this.zoomHelper.level + this.scrollPosition.x;
...
Game.prototype.handleMouseDown = function (e) {
   ...
   switch (Tools.current) {
      ...
      case Tools.DEMOLISH:
         ...
         if (
            this.tileMap[pos.row] != undefined &&
...
Game.prototype.translatePixelsToMatrix = function (x, y) {
   ...
   gridOffsetX +=
      this.canvas.width / 2 -
      (tileWidth / 2) * this.zoomHelper.level +
      this.scrollPosition.x;
...
Game.prototype.draw = function (srcX, srcY, destX, destY) {
   ...
   var pos_BR = this.translatePixelsToMatrix(
      this.canvas.width,
      this.canvas.height
   );
   ...
   for (var row = startRow; row < rowCount; row++) {
      for (var col = startCol; col < colCount; col++) {
         var xpos =
            (row - col) * tileHeight + this.grid.width * this.zoomHelper.level;
         xpos +=
            this.canvas.width / 2 -
            (tileWidth / 2) * this.zoomHelper.level +
            this.scrollPosition.x;
         ...
         var ypos =
            (row + col) * (tileHeight / 2) +
            this.grid.height * this.zoomHelper.level +
            this.scrollPosition.y;
         ...
         if (this.tileMap[row] != null && this.tileMap[row][col] != null) {
            ...
            if (
               Math.round(xpos) + tileWidth >= srcX &&
